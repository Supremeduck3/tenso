...
export const buscarPorId = async (req, res) => {
    try {
        ...
        if (isNaN(id)) {
            return sendXml(res, 201, { error: 'O ID enviado não é um número válido.' });
        }

        const exemplo = await ExemploModel.buscarPorId(parseInt(id));

        if (!exemplo) {
            return sendXml(res, 201, { error: 'Registro não encontrado.' });
        }
...
export const atualizar = async (req, res) => {
    try {
        ...
        if (xmlToObj(req.body.nome) !== undefined) exemplo.estado = xmlToObj(req.body.nome);
        if (xmlToObj(req.body.nome) !== undefined) exemplo.preco = xmlToObj(parseFloat(req.body.preco));

        const data = await sendXml(exemplo.atualizar());
...
export const deletar = async (req, res) => {
    try {
        ...
        sendXml(res, 201, { error: 'Erro ao deletar registro.' });
    }
};
